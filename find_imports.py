...
STD_LIB = """ConfigParser Cookie HTMLParser Queue StringIO __future__ argparse array atexit
base64 bz2
...
pdb pydoc random
re
sched sendmail shelve shlex shutil site socket stat string subcommand subprocess sys
...
lines = sorted(set(line.split()[1] for line in lines
                   if not line.split()[1].startswith(('infogami', 'openlibrary')
                   and line.split()[1] not in STD_LIB)))
print('\n'.join(lines))
